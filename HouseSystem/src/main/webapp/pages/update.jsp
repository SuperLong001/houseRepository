...
var n = new Array(19);
	...
	$(function(){
		...
		var house_district = "${sessionScope.UPDATE_HOUSE.house_district}";
		
		$("#qxid").val(house_district);
		
		selectjd();
		
		var house_street = "${sessionScope.UPDATE_HOUSE.house_street}";
		
		$("#jdid").val(house_street);
		
		var room = "${sessionScope.UPDATE_HOUSE.room}";
		
		$("#room").val(room);
		
		var hall = "${sessionScope.UPDATE_HOUSE.hall}";
		
		$("#hall").val(hall);
		
		var house_type = "${sessionScope.UPDATE_HOUSE.house_type}";
...
-->
	...
	<form action="../house/updateHouse.action?submit=1&houseId=0" method="post" name="mf" onsubmit="return validateTitle()">
		<input type="hidden" name="affirm" value="n"/>
		<table>
			<tr style="display: none;">
				<input type="text" style="display: none;" value="${sessionScope.UPDATE_HOUSE.house_id}"  name="house_id" />
				
				<input type="text" style="display: none;" value="${sessionScope.UPDATE_HOUSE.release_time}" name="release_time"/>
			</tr>
			...
			<tr>
			  <td><strong>标题：</strong></td>
				<td><input type="text"  value="${sessionScope.UPDATE_HOUSE.house_title}" name="house_title"></td>
			</tr>
			<tr>
			  <td><strong>区县：</strong></td>
				<td><select name="house_district" id="qxid" onChange="selectjd()">
...
<option value='18'>平谷</option>
			...
			<tr>
			  <td><strong>街道：</strong></td>
				<td><select name="house_street" id="jdid">
					<option value="0">不限--</option>
				</select></td>
			</tr>
			<tr>
			  <td><strong>租金：</strong></td>
				<td><input type="text" value="${sessionScope.UPDATE_HOUSE.rental_price}" name="rental_price"/>元/月</td>
			</tr>
			<tr>
			  <td><strong>户型：</strong></td>
				...
				</select>室<select id="hall" name="hall">
					<option value="1">1</option>
					<option value="2">2</option>
					<option value="3">3</option>
					<option value="4">4</option>
			...
			<tr>
			  <td><strong>房屋类型：</strong></td>
				<td><select id="house_type" name="house_type">
					<option value='0'>不限--</option>
...
<option value='5'>别墅</option>
			...
			<tr>
			  <td><strong>联系电话：</strong></td>
				<td><input type="text" value="${sessionScope.UPDATE_HOUSE.contacts_phone}" name="contacts_phone"></td>
			</tr>
			<tr>
			  <td><strong>联系人：</strong></td>
				<td><input type="text" value="${sessionScope.UPDATE_HOUSE.contacts_name}" name="contacts_name"></td>
			</tr>
			<tr>
			  <td><strong>房屋信息：</strong></td>
				<td><textarea name="house_description" rows="10" cols="40">${sessionScope.UPDATE_HOUSE.house_description}</textarea></td>
			</tr>
			<tr>
				<td colspan="2">
