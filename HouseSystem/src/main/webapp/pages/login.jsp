...
-->
	...
	<tr>
		<td width=''><font color='red'>当前用户：
			<c:out value="${sessionScope.USER_INFO.userName}"></c:out>
			</font>
		</td>
	</tr>
	<tr>
		<td width='150'><a href="../house/queryMyHouse.action">管理我的租房信息</a></td>
	</tr>
	<tr>
		<td width='100'><a href="post.jsp">发布租房信息</a></td>
	</tr>
	<tr>
		<td width='100'><a href="../user/writeroff.action">[注销]</a></td>
	</tr>
	</table>
	...
	if (o){
		if (o.style.display == "none"){ o.style.display = "block"; } else {o.style.display = "none";}
				...
				<div style="text-align:left;width:88%;">
					...
					<form action="../house/dimQuery.action" method="post" >
						<input type="text" name="house_title"/>
		...
		<form action="../house/advancedQuery.action" method="post">
		<table width="88%;" id="advSearch" style="border:solid 0px #000;display:none;">
		
			
<tr>
				<td>区县：</td>
				<td><select name="district" id="qxid" onChange="selectjd()" style="width:80">
...
<option value='18'>平谷</option>
			...
			<tr>
				<td>街道：</td>
				<td><select name="street" id="jdid" style="width:80">
					<option value="0">不限--</option>
				</select></td>
			</tr>
			<tr>
				<td>租金：</td>
				<td><input type="text" name="min_price" size="5" class="unnamed1" > 至 <input type="text" name="max_price" size="5" class="unnamed1" > 元/月</td>
			</tr>
			<tr>
				<td>户型：</td>
				<td><select name="room"  style="width:60">
					<option value="0">不限--</option>
			...
			<tr>
				<td>房屋类型：</td>
				<td>
<input type="checkbox" value="1" name="house_types[0]">地下室
<input type="checkbox" value="2" name="house_types[1]">平房
<input type="checkbox" value="3" name="house_types[2]">普通住宅
<input type="checkbox" value="4" name="house_types[3]">公寓
<input type="checkbox" value="5" name="house_types[4]">别墅
</td>
			</tr>
			<tr>
				<td>发布日期：</td>
				<td><select name="release_time" style="width:80">
					<option value="0">不限--</option>
			...
			<tr>
			<td><input type="submit" value="搜&nbsp索"/></td>
			</tr>
<tr>
...
<table width="88%" cellspacing="0">
				...
				<c:forEach items="${sessionScope.PAGING_DATA}" var="house">
					<tr>
						<td><a href='../house/houseDetail?houseId=${house.house_id}'>${house.house_title}</a></td>
						<td align='center' style='height:30px;'>${house.rental_price}元</td>
						<td align='center'>${house.release_time}</td>
					</tr>
				</c:forEach>
				...
				<tr>
					...
					<c:forEach begin="1" end="${sessionScope.MAX_PAGE}" varStatus="i">
						<c:choose>
							<c:when test="${sessionScope.CURRPAGE==i.count}">
								<strong>
									<span class="fk fk_cur"></span><span class="pc">${i.count}</span>
								</strong>
							</c:when>
							
							<c:otherwise>
								<a href="../house/pagingQuery.action?currPage=${i.count}">
									<span class="fk fkd"></span><span class="pc">${i.count}</span>
								</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
