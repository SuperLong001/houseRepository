...
var district = new Array('不限--','海淀','朝阳','崇文','宣武','东城','西城','通州','丰台','昌平','大兴','门头沟','石景山','房山','顺义','密云','怀柔','延庆','平谷');

var houseType = new Array('不限--','地下室','平房','普通住宅','公寓','别墅');
...
n[18] = new Array('滨河','兴谷','渔阳','峪口','马坊','金海湖','平谷镇','大兴庄','马昌营','东高村','夏各庄','南独乐河','镇罗营','大华山','刘家店','王辛庄','山东庄','黄松峪','熊儿寨','其他');
	...
	$(function(){
		...
		var d = parseInt($("#district").html());
		
		var k = parseInt($("#street").html());
		
		if(d!=0){
			
			$("#district").html(district[d]);
		...
		$("#house_type").html( houseType[ parseInt($("#house_type").html()) ] );
...
-->
	...
	<table width="460">
	 ...
	 <tr>
		<td id="listTitle">${sessionScope.HOUSE_DETAIL.house_title}</td>
	  </tr>
	  <tr>
	    <td id="listTitle"><hr/></td>
	    </tr>
	  <tr>
		<td><strong>电话/手机：</strong>${sessionScope.HOUSE_DETAIL.contacts_phone}</td>
	  </tr>
	  <tr>
		<td><strong>联系人：</strong>${sessionScope.HOUSE_DETAIL.contacts_name}</td>
	  </tr>
	  <tr>
		<td><strong>房屋类型：</strong><span id="house_type">${sessionScope.HOUSE_DETAIL.house_type}</span></td>
	  </tr>
	  <tr>
		<td><strong>户型：</strong>${sessionScope.HOUSE_DETAIL.room}室${sessionScope.HOUSE_DETAIL.hall}厅</td>
	  </tr>
	  <tr>
		<td><strong>价格：</strong>${sessionScope.HOUSE_DETAIL.rental_price}元</td>
	  </tr>
	  <tr>  
		<td><strong>地段：</strong><span id="district">${sessionScope.HOUSE_DETAIL.house_district}</span>&nbsp;&nbsp;<span id="street">${sessionScope.HOUSE_DETAIL.house_street}</span></td>
	  </tr>
	  <tr>
		<td><strong>发布时间：</strong> ${sessionScope.HOUSE_DETAIL.release_time}</td>
	  </tr>
	  <tr>
		<td><hr></td>
	  </tr>
	  <tr>
		<td >${sessionScope.HOUSE_DETAIL.house_description}</td>
	  </tr>
	  <tr>
		<td><hr></td>
	  </tr>
	  <tr>
		<td>
		<form action="../house/confirmSubmitHouse.action" method="POST">
